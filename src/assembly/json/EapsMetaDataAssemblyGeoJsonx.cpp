...
namespace eap {
	namespace sma {
		...
		{
			return MetaDataAssemblyGeoJsonxPtr(new MetaDataAssemblyGeoJsonx());
		}
		...
		std::string MetaDataAssemblyGeoJsonx::assembly()
		{
			if (_have_basic) {
				...
				if(_meta_data_basic.GimbalPayloadInfos_p.ImageProcessingBoardInfo_p.AiInfos_p.AiStatus == 1 && ai_data_detc_size > 0)
				{
				...
				return eap::sma::jsonToString(entity_root);
