...
namespace eap {
	namespace sma {
		static std::array<char, AV_ERROR_MAX_STRING_SIZE> FFErrorPtr;
		...
		{
			return std::string(av_make_error_string(FFErrorPtr.data(), AV_ERROR_MAX_STRING_SIZE, error_num));
		}
		...
		*/
		...
		int h264_mp4toannexb(AVFormatContext* fmt_ctx, AVPacket* in, AVPacket** out)
		{
			...
			do {
				...
				if (unit_type == 5) {
					...
					// 非关键帧只需要添加特征码
					if ((ret = alloc_and_copy(_out, NULL, 0, buf, nal_size)) < 0)
						goto fail;
				}
				...
				if(spspps_pkt.data){
					av_free(spspps_pkt.data);
					spspps_pkt.data = nullptr;
				}
		...
		fail:
			av_packet_free(&_out); // 凡是中途处理失败退出之前都要将促使的out释放，否则会出现内存泄露
			*out = in;
